...
| Scenario | Shape | Patterns shown |
|----------|-------|----------------|
| `EligibilityDecision` | Single hierarchical boolean decision | IfElse, Expression, DecisionTable, RuleBased, StateMachine |
| `RoutingPipeline` | Seven sub-decisions in sequence, one routing output | Pipeline (orchestrator) + BitmaskLookup (final step) |
...
### Stage 2 — Navigate the canvas with the needle

Every workflow has exactly one activity marked with the color **#cb39fe**. This is the *needle* — the single activity where the consequential decision is encoded.

Teaching prompt: *"Find the purple activity first. Everything else supports it."*

The needle makes the logical core of each workflow immediately locatable regardless of overall workflow size or nesting depth.
...
> "[NEEDLE] Expression: If(in_VatId.StartsWith("DOM-"), "NATIONAL", "INTERNATIONAL")"

Use these annotations to direct attention to the decision-encoding activity and explain the business reasoning behind it.

### The needle convention

- Every workflow has **exactly one** needle.
- The needle is always on the activity that encodes the consequential decision — the one that, if changed, would change outputs.
- Supporting activities (extraction, passthrough, logging) are never the needle.

If a learner cannot identify the needle unaided, that is a signal they have not yet understood which activity encodes the decision logic.
...
|---------|----------------|
| IfElse | Nested `If` activities, one predicate per node |
| Expression | Single compound VB.NET expression in `If.Condition` |
| DecisionTable | Inline `List(Of String())` with wildcard cells, LINQ `.Any()` |
| RuleBased | Named Boolean variables per rule, combined with `OrElse` |
| StateMachine | WF4 `StateMachine` with explicit state graph |
| BitmaskLookup | Booleans × powers-of-2 → integer index → lookup table |
| Pipeline | Sequential sub-decisions, intermediate results as local variables |
...
4. Place the needle annotation (`#cb39fe`) on exactly one activity per workflow.
